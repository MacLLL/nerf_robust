...
def main(unused_argv):
  ...
  for step, batch in zip(range(init_step, num_steps + 1), pdataset):
    ...
    # ######## 从训练代码中拿出residual：
    if config.data_loss_type == 'gmmrobust' and config.train_residual:
    ...
    if config.train_residual and config.fit_gmm_train_residual_every > 0 and step % config.fit_gmm_train_residual_every == 0 and step > (config.pretrain_steps-1) : # 
      '''
      在训练过程中统计residual的分布情况, 并拟合gmm_model。
      注意在拟合gmm过程中是不需要mask的, 模拟真实应用场景中没有对比图相对的情况。
      '''
    ...
    if config.test_residual and config.fit_gmm_test_residual_every > 0 and step % config.fit_gmm_test_residual_every == 0 and step > (config.pretrain_steps-1): # and step > 49990 
      '''
      在训练过程中统计residual的分布情况, 并拟合gmm_model。
      注意在拟合gmm过程中是不需要mask的, 模拟真实应用场景中没有对比图相对的情况。
      '''
